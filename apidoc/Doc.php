...
class Doc
{
    ...
    {
        $defConf = include __DIR__ . '/config.php';
        $this->config = array_merge($defConf, $config);
    }
    /**
     * 使用 $this->name 获取配置
     * @access public
    ...
    public function getList()
    {
        ...
        foreach ($controller as $class) {
            if (class_exists($class)) {
                ...
                $filter_method = array_merge([
                    '__construct'
                ], $this->config['filter_method']);
                $module['actions'] = [];
    ...
    public function getInfo($class, $action)
    {
        ...
        if ($class && class_exists($class)) {
            ...
            $action_doc['title'] = $action_doc['title'] ?? '请设置title注释';
            $action_doc['url'] = $action_doc['url'] ?? '请设置url注释';
            $action_doc['method'] = $action_doc['method'] ?? 'GET'; // GET
            $action_doc['method'] = strtoupper($action_doc['method']);

            $action_doc['author'] = $action_doc['author'] ?? '请设置auhtor注释';
            $action_doc['description'] = $action_doc['description'] ?? '';

            if (isset($action_doc['header'])) {
                $action_doc['header'] = $this->parseDocData($action_doc['header']);
            }
            if (isset($action_doc['param'])) {
                $action_doc['param'] = $this->parseDocData($action_doc['param']);
            }
        }
        return $action_doc;
    }
    protected function parseDocData($data = [])
    {
        foreach ($data as $key => $val) {
            $data[$key]['name'] = $val['name'] ?: '-';
            $data[$key]['name_key'] = $val['name'] ?: '';
            $data[$key]['type'] = $val['type'] ?? '-';
            $data[$key]['require_str'] = $val['require'] == 1 ? '必填' : '非必填';
            $data[$key]['default'] = $val['default'] ?: '-';
            $data[$key]['other'] = $val['other'] ?? '-';
                ...
                foreach ($method as $action) {
                    if (!in_array($action->name, $filter_method)) {
                        ...
                        if ($doc_str) {
                            $action_doc = $doc->parse($doc_str);
                            $action_doc['name'] = $class . "::" . $action->name;
                            if ((isset($action_doc['title']) && strpos($action_doc['title'], $keyword) !== false) || (isset($action_doc['description']) && strpos($action_doc['description'], $keyword) !== false) || (isset($action_doc['author']) && strpos($action_doc['author'], $keyword) !== false) || (isset($action_doc['url']) && strpos($action_doc['url'], $keyword) !== false)) {
